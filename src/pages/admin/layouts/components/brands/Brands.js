...
const Brands = () => {
  ...
  const getbranddata = async () => {
    ...
    try {
      const response = await axios({
        method: "post",
        url: "http://localhost:8000/api/getbranddata",
  ...
  const deleteuser = async (id) => {
    const deleteres = await axios({
      method: "delete",
      url: `http://localhost:8000/api/deletebranddata/${id}`,
    });
    ...
    <div className="bgwhite border-d mtpx9 cust-scroll p20">
      ...
      <div className="">
        <div className="mtpx5 mbpx15 flex justify-between items-center">
          <h5 className="fsize20 textprimary mtpx1 mbpx1 font-600">Brands</h5>
          <button
            onClick={() => setbrandsidebar(true)}
